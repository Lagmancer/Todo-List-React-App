  ...
  return (
    ...
    >
      ...
      <div className="absolute bottom-0 left-0 container mx-auto w-[25%] md:w-[25%] lg:w-[20%] px-5 py-5 h-[85%] bg-[#000000] rounded-md flex flex-col items-center">
        ...
        <button className="mt-2 group flex items-center h-[8%] space-x-7 w-full rounded-xl px-6 py-2 bg-white text-[#FF6767] transition cursor-pointer">
          <img
            src="/Dashboard/VitalTask-Red.png"
            alt="vitaltask icon"
            className="w-[2.5%] h-[50%]"
          />
            ...
            {tasks.length === 0 ? (
              ...
              (() => {
                ...
                const sortedTasks = [...tasks].sort((a, b) => {
                  const aPriority =
                    priorities.find((p) => p.id === a.priority)
                      ?.priority_level || 0;
                  const bPriority =
                    priorities.find((p) => p.id === b.priority)
                      ?.priority_level || 0;
